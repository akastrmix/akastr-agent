...
这些能力可以组合；`target` 和 `runner` 不是不同二进制，也不是协议中的永久角色。正式部署仍建议将 IPQuality 放在专用 Runner，避免资源占用和目标网络变化互相影响。
...
v0.1.0 的自然变化 monitor 实际只产生 IPv4 事件：首次成功观察只建立基线，不上报；之后的变化先写入 `ip_state_file`，再发送 `ip.observed`。收到主控的 `ip.observed_ack` 之前，同一事件会在重连后继续重试。因此 `observe_ipv6` 在 v0.1.0 应保持 `false`；当前运行时不会生成自然 IPv6 变化事件。
...
目标节点的 capability metadata 可以公布地址来源和端口，但绝不包含用户名或密码。Runner 上的凭据位于独立 root-only profile 文件，以稳定 profile ID 索引。

Runner 使用指定目标的 SOCKS5 端点运行固定版本、固定 SHA-256 的 IPQuality 脚本。执行前后都会通过 SOCKS5 观察 IPv4，并与任务中的预期目标 IPv4 代际比对。代际在完成前变化时，即使脚本退出成功，AkastrCloud 也不会把结果作为当前代际的有效报告。

官方脚本在仅 IPv4 模式下可能已经生成有效报告 URL，却返回非零 Bash 状态。因此 v0.1.0 将“输出中包含有界、有效的 `https://report.check.place/...` URL，且代理 postflight 成功”视为完成；非零退出且没有报告 URL 仍是 `script_failed`。
...
截至本文更新时，生产 Agent WSS Gate 仍关闭，安装记录和 command 均为零。操作者必须等待主控明确放行，再按 [安装、配置与使用教程](INSTALLATION.md) 逐台迁移；不得因 Agent 二进制已发布就停用旧服务。
