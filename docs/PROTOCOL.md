...
- `command_type`：v0.8.2 runtime 只接受 `changeip.execute` 和 `ipquality.execute`；
...
- v0.8.2 是 Agent release/bootstrap 版本；SOCKS5 capability 只允许 `port`，不接受 `address_source` 或 `advertised_host`。
